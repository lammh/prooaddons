        ...
        class Payslips(BrowsableObject):
            ...
            def sum(self, code, from_date, to_date=None):
                ...
                self.cr.execute("SELECT sum(case when hp.credit_note = False then (pl.total) else (-pl.total) end)\
                            FROM hr_payslip as hp inner join hr_payslip_line as pl on hp.id = pl.slip_id \
                            inner join hr_contract hc on hc.id = hp.contract_id \
                            inner join hr_contract_type hct on hct.id = hc.type_id\
                            WHERE hp.employee_id = %s AND hp.state = 'done' and hct.taxable=True \
                            AND hp.date_from >= %s AND hp.date_to <= %s AND hp.id = pl.slip_id AND pl.code = %s",
                            (self.employee_id, from_date, to_date, code))
                res = self.cr.fetchone()
                return res and res[0] or 0.0

            def sum_old(self, code, from_date, to_date):
                Date = datetime.datetime.strptime(to_date, "%Y-%m-%d")
                to_date = (Date + relativedelta.relativedelta(day=1,days=-1)).strftime('%Y-%m-%d')
                self.cr.execute("SELECT sum(case when hp.credit_note = False then (pl.total) else (-pl.total) end)\
...
#         structure_ids = self.pool.get('hr.contract').get_all_structures(cr, uid, contract_ids, context=context)
        ...
        rule_ids = self.pool.get('hr.payroll.structure').get_all_rules(cr, uid, [payslip.struct_id.id], context=context)
