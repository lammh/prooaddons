...
class oschool_extra__payment(osv.osv_memory):
    _name = 'oschool.extra.payment'
    ...
    def button_payment(self, cr, uid, ids, context=None):
        ...
        for line in self.browse(cr, uid, ids, context=context):
            ...
            if pos_line_ref.browse(cr, uid, l[0], context=context).order_id:
                ...
                journal_extra = journal_obj.search(cr, uid, [('extra', '=', True),
                                                             ('company_id','=',pos_ref.browse(cr, uid,inv_id).company_id.id)
                                                             ], limit=1, context=context)
                if journal_extra:
                    cr.execute('update pos_order set sale_journal = %s where id = %s', (journal_extra[0], inv_id))
                for pos_line in l:
                    pos_line_ref.write(cr, uid, pos_line, {'order_id': inv_id}, context=context)
        pos_ref.write(cr, uid, inv_id, {'type':'Extra'})
