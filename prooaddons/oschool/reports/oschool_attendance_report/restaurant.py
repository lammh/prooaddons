...
class restaurant_wizard(models.Model):
    _name = 'oschool.restaurant.wizard'

    class_id = fields.Many2one('oschool.classes', string="Zone")
    period_id = fields.Many2one('account.period', 'Period')
    month = fields.Char(compute="_get_month")
    presence_ids = fields.One2many('oschool.student_restaurant_presence','report_id')
    ...
    def create(self, cr, uid, vals):
        ...
        for id in presence_ids:
            r.append([4,id])
        vals['presence_ids'] = r
        return  super(restaurant_wizard, self).create(cr, uid, vals)