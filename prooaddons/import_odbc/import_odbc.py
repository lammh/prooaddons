...
class import_odbc_dbtable(orm.Model):
    ...
    def _import_data(self, cr, uid, flds, data, model_obj, table_obj, log):
        ...
        if table_obj.raise_import_errors:
            ...
            try:
                code, resource, message, unused = model_obj.import_data(cr, uid, cols, [data], noupdate=table_obj.noupdate)
                cr.commit()
                if code == -1:
                    errmsg = message
            except:
    ...
    def import_run(self, cr, uid, ids=None, context=None):
        ...
        for action_ref in actions:
            ...
            params = {'sync': sync}
            sql_source = obj.sql_source.replace('sync',"'" + str(sync) + "'")
            res = db_model.execute(cr, uid, [obj.dbsource_id.id],
                                   sql_source, params, metadata=True)
